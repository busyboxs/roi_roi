...
class ProposalTargetLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        top[0].reshape(1, 5)  # rois
        top[1].reshape(1, 5)  # P3
        top[2].reshape(1, 5)  # P4
        top[3].reshape(1, 5)  # P5
        # labels
        top[4].reshape(1, 1)
        # bbox_targets
    ...
    def forward(self, bottom, top):
        ...
        p3_rois = bottom[0].data
        p4_rois = bottom[1].data
        p5_rois = bottom[2].data
        ...
        rois = np.vstack((rois_p3, rois_p4, rois_p5))
        labels = []
        labels = labels_p3.tolist() + labels_p4.tolist() + labels_p5.tolist()
        labels = np.array(labels)
        bbox_targets = np.vstack((bbox_targets_p3, bbox_targets_p4, bbox_targets_p5))

        bbox_inside_weights = np.vstack((bbox_inside_weights_p3, bbox_inside_weights_p4, bbox_inside_weights_p5))
        ...
        bbox_outside_weights = np.vstack((outside_weight_p3, outside_weight_p4, outside_weight_p5))
        ######################################################
        # sampled rois
        # print labels
