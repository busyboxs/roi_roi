...
def get_ohem_minibatch(loss, rois, labels, bbox_targets=None,
    ...
    length_roi = len(rois)
    rois_3, rois_4, rois_5 = np.vsplit(rois, 3)
    assert len(rois_3) == length_roi / 3
    assert len(rois_4) == length_roi / 3
    assert len(rois_5) == length_roi / 3
    ...
    for roi_ in rois_hard:
        if roi_ in rois_3:
            rois_hard_3.append(roi_)
        if roi_ in rois_4:
            rois_hard_4.append(roi_)
        if roi_ in rois_5:
            rois_hard_5.append(roi_)

    blobs = {'rois_hard_3': rois_hard_3,
             'rois_hard_4': rois_hard_4,
             'rois_hard_5': rois_hard_5,
             'rois_hard'  : rois_hard,
